#!/usr/bin/env python3
"""
Generate a sitemap.xml for the Spanish learning app.

- Reads spanishWords.csv and spanishStories.csv
...
# --- CONFIGURATION ---
SITE = "https://osloandrew.github.io/spanish"
WORDS_CSV = Path("spanishWords.csv")
STORIES_CSV = Path("spanishStories.csv")
...
def build_story_urls(csv_path: Path) -> list[str]:
    ...
    with csv_path.open(newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            title = (row.get("titleSpanish") or "").strip()
            if not title:
                continue
